...
$(document).ready(function () {
    ...
    for (let i = 1; i <= 100; i++) {
        let row = $(`<div class ="cell-row"></div>`); /// first create row  then we traverse coloumn and creating cells then append these cells into row and last append row in a container
        for (let j = 1; j <= 100; j++) {
   ...
   function updateCell(property,value,defaultPossible){
       $(".input-cell.selected").each(function(){
           ...
           let [rowId,coldId] = getRowCol(this);
           if(cellData[selectedSheet][rowId]){
               if(cellData[selectedSheet][rowId][coldId]){
                cellData[selectedSheet][rowId][coldId][property] = value;
               }
               else{
                cellData[selectedSheet][rowId][coldId]  = {...defaultPossible};
                cellData[selectedSheet][rowId][coldId][property] = value;
               }
           }
           else{
